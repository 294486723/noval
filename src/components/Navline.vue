<template>
  <div class="navline">
      <router-link  to="/bookshelf" class="nav-one"><i class="nav-two-1"></i><p>书架</p></router-link>
      <router-link to="/ranklist" class="nav-one"><i class="nav-two-2"></i><p>排行</p></router-link>
      <router-link to="/VIP" class="nav-one"><i class="nav-two-3"></i><p>VIP</p></router-link>
      <router-link to="/Recharge" class="nav-one"><i class="nav-two-4"></i><p>充值</p></router-link>
  </div>
</template>

<script>
export default {

}
</script>

<style scoped>
.navline{
     text-align: center;
    background-color: #fff;
    width: 100%;
    padding: 0 10px;
    display: flex;
    box-sizing: border-box;
}
.nav-one{
    width: 25%;
    padding: 15px 0;
    height: 100px;
    box-sizing: border-box;
   
}
.nav-one i{
    display: block;
    height: 45px;
    margin-bottom: 10px;
}
.nav-one p{
    font-size: 12px;
    color: #333;
}
.nav-two-1{
    background: url(data:image/png;base64,)50% no-repeat;
    background-size: contain;
}
.nav-two-2{
    background: url(data:image/png;base64,)50% no-repeat;
    background-size: contain;
}
.nav-two-3{
    background: url(data:image/png;base64,)50% no-repeat;
    background-size: contain;
}
.nav-two-4{
    background: url(data:image/png;base64,)50% no-repeat;
    background-size: contain;
}
</style>